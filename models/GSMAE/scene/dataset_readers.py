...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", objects_folder=None):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        if "camera_angle_x" in contents:
            ...
            for idx, frame in enumerate(frames):
                ...
                objects_accpatch = None
                objects = None
                cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], fid=0))
        else:
            frames = contents["frames"]
            for idx, frame in enumerate(frames):
...
# def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", objects_path=None):
#     cam_infos = []

#     print("transformsfile is: ", transformsfile)
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", objects_path=None):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension, objects_path)
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension, objects_path)
    ...
    num_pts = 50_000
    xyz = np.random.randn(num_pts, 3) * 1.0 / 16
    xyz = np.random.randn(num_pts, 3) * 1.0 / 16
...
def readNerfSyntheticInfo_Force(path, 
                                white_background, 
                                eval, 
                                extension=".png", 
                                objects_path=None,
    ...
    # print("test_poses is: ", test_poses)
    # print("w2cs_test is: ", w2cs_test)
    # exit(0)
...
sceneLoadTypeCallbacks = {
    ...
    "Blender_Force": readNerfSyntheticInfo_Force,

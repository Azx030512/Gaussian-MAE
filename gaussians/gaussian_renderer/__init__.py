...
def render_phygs(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling, particle_F, is_6dof=False,
    ...
    # cov correction
    N = cov3D_precomp.shape[0]
    cov3D = torch.zeros((N, 3, 3), dtype=cov3D_precomp.dtype, device=cov3D_precomp.device)
...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling, is_6dof=False,
           scaling_modifier=1.0, override_color=None, return_depth=False, return_normal=False, return_opacity=False):
    """
    Render the scene. 
